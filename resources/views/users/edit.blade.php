                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" required
                                   {{ $user->authorized_email ? 'readonly' : '' }}
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('email') border-red-500 @enderror {{ $user->authorized_email ? 'bg-gray-100 cursor-not-allowed' : '' }}">
                            @if($user->authorized_email)
                                <p class="mt-1 text-xs text-gray-500">
                                    <svg class="w-3 h-3 inline mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"/>
                                    </svg>
                                    Email is locked to this role and cannot be changed
                                </p>
                            @endif
                            @error('email')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        @else
                        <!-- System Admin: Company locked to their company -->
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700">Company</label>
                            <input type="text" value="{{ $user->company->name ?? 'N/A' }}" disabled
                                   class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 shadow-sm sm:text-sm">
                            <p class="mt-1 text-xs text-gray-500">Company cannot be changed</p>
                            ...
                            <select name="role" id="role" required
                                ...
                                @foreach($roles as $role)
                                    @if($role->name !== 'Employee')
                                        @php
                                            $userHasRole = $user->roles->contains('name', $role->name);
                                        @endphp
                                        <option value="{{ $role->name }}" {{ (old('role') == $role->name || $userHasRole) ? 'selected' : '' }}>
                                            {{ $role->name }}
                                        </option>
                                    @endif
                                @endforeach
                        ...
                        <div>
                            <label for="password" class="block text-sm font-medium text-gray-700">New Password <span class="text-gray-500">(leave empty to keep current)</span></label>
                            <div class="relative">
